...
@ContextConfiguration(classes = {EmailPasswordLoginService.class})
@ExtendWith(SpringExtension.class)
class EmailPasswordLoginServiceDiffblueTest {
    @Autowired
    ...
    /**
     * Method under test: {@link EmailPasswordLoginService#login(Login)}
     */
    @Test
    void testLogin() {
        User user = new User();
        ...
        when(userRepository.findOneByEmail(Mockito.<String>any())).thenReturn(ofResult);

        Login login = new Login();
        login.setLoginType(1);
        login.setPassword("iloveyou");
        login.setUsername("janedoe");
        User actualLoginResult = emailPasswordLoginService.login(login);
        verify(userRepository).findOneByEmail(Mockito.<String>any());
