...
@ContextConfiguration(classes = {PhonePasswordLoginService.class})
@ExtendWith(SpringExtension.class)
class PhonePasswordLoginServiceDiffblueTest {
    @Autowired
    ...
    /**
     * Method under test: {@link PhonePasswordLoginService#login(Login)}
     */
    @Test
    void testLogin() {
        User user = new User();
        ...
        when(userRepository.findOneByPhone(Mockito.<String>any())).thenReturn(ofResult);

        Login login = new Login();
        login.setLoginType(1);
        login.setPassword("iloveyou");
        login.setUsername("janedoe");
        User actualLoginResult = phonePasswordLoginService.login(login);
        verify(userRepository).findOneByPhone(Mockito.<String>any());
