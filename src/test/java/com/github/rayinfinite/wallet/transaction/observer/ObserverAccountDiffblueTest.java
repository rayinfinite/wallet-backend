...
@ContextConfiguration(classes = {ObserverAccount.class})
@ExtendWith(SpringExtension.class)
class ObserverAccountDiffblueTest {
    @MockBean
    ...
    /**
     * Method under test: {@link ObserverAccount#transaction(Transaction, Category, BigDecimal)}
     */
    @Test
    void testTransaction() {
        ...
        doNothing().when(accountService).save(Mockito.<Account>any());
        ...
        category2.setType(1);
        observerAccount.transaction(transaction, category2, new BigDecimal("2.3"));
        verify(accountService).get(Mockito.<Long>any());
        verify(accountService).save(Mockito.<Account>any());
    }
}
